...
async function extraerInfoRepositorio() {
  ...
  try 
  {
    ...
    let vigilante = chokidar.watch(archivoDescargadoPath); // Indico que vigile un archivo

    // Evento que se ejecutara cuando detecte la creación del archivo
    vigilante.on('add', function() 
    { 
      ...
      vigilante.close();    // Dejo de vigilar - Termina el evento
    });
